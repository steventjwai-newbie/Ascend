...
test("server-renders the redacted Q1 evidence dashboard", async () => {
  ...
  assert.match(html, /Alpha Owner Control Tower/);
  assert.match(html, /Daily Control Tower/);
  assert.match(html, /Investigations/);
  assert.match(html, /Cost Explorer/);
  assert.match(html, /Menu margin quadrant/);
  assert.match(html, /51\.93%/);
  assert.match(html, /24\.71%/);
  ...
  assert.match(html, /1\.079/);
  assert.match(html, /retained coproduct/i);
